...
Функция ПолучитьПараметрыПечатиСчетаЗаказа(ПечДокумент) Экспорт	
	...
	|	ДоговорКонтрагента,
	|	Организация,
	|	ВЫБОР КОГДА Грузополучатель=Значение(Справочник.Контрагенты.ПустаяСсылка) ТОГДА Контрагент Иначе Грузополучатель КОНЕЦ КАК Получатель,
	...
	ПараметрыПечати.Вставить("ПредставлениеПолучателя", ФормированиеПечатныхФорм.ОписаниеОрганизации(УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Шапка.Получатель, Шапка.Дата), "ПолноеНаименование,ИНН,КПП,ФактическийАдрес,Телефоны,"));
