...
Функция ПечатьДокументаМЛ(ВыбДокумент)
	...
	Для Каждого РезультатЗапроса из ТаблицаРеализаций Цикл
		...
		Если Накладная.Организация<>Справочники.ИТИКонстанты.ОргГарц.Указатель И 
			(Накладная.ОтражатьВУправленческомУчете	И Накладная.ОтражатьВБухгалтерскомУчете	И Накладная.ОтражатьВНалоговомУчете) Тогда 
		...
		//20111219 Лямкина Юлия 
		Если ТипЗнч(Накладная)=Тип("ДокументСсылка.ВозвратТоваровПоставщику") ИЛИ ЗначениеЗаполнено(Накладная.АдресДоставки)=Ложь Тогда
